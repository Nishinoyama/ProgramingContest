...
void dfs1( int from, int pare ){
    subN[from] = 1;
    for( int to : e[from] ){
        if( to == pare ) continue;
        dfs1(to, from);
        subN[from] += subN[to];
        d[from].push_back( subN[to] );
    }
}

void dfs2( int from, int pare, int dpare ){
    int cnt = dpare;
    // ans
    for( int dN : d[from] ){
        cnt += dN;
    }
    ans[from] += modint(1)/PowMod(2,cnt); // all white
    // one black other white
    ans[from] += modint(1)*(PowMod(2,dpare)-1)/PowMod(2,cnt);
    for( int dN : d[from] ){
        ans[from] += modint(1)*(PowMod(2,dN)-1)/PowMod(2,cnt);
    }
    ans[from] = modint(1) - ans[from];

    // dfs
    cnt = 0;
    for( int to : e[from] ){
        if( to == pare ) continue;
        dfs2(to, from, dpare+subN[from]-d[from][cnt++] );
    }
}
...
int main() {
    ...
    subN.assign(N,0);
    ans.assign(N,modint(0));
    d.resize(N);

    dfs1(0,-1);

    Rep(i,N){
//        for( int c : d[i] ) printf ("%d:", c );
//        printf ("\n");
    }
