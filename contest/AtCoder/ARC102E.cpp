...
int main() {
    ...
    vector<modint> f(NK+1,1);
    Rep(i,NK) f[i+1] = f[i] * (i+1);

    auto c = [&]( int n, int r ){ assert(n>=r); return f[n]/f[r]/f[n-r]; };

    modint all = f[N+K-1] / f[N] / f[K-1];

    vector<modint> ans(K*2+1,0);
    Repe(i,K+2,2){
        ans[i] = 0;
        Rep(j,N/2){
            ans[i] -= modint(j%2?1:mod-1) * PowMod(K,N-2*j-2) * ((i-j*2)/2);
        }
        ans[i] = all - ans[i];
        ans[2*K-i+2] = ans[i];
    }
