...
int main() {
    ...
    Rep(i,N*2) f[i+1] = f[i] * (i+1);

    modint a = modint(A)/100, b = modint(B)/100;

    modint da = a/(a+b);
    modint db = b/(a+b);
    modint dc = modint(1)/(a+b);
    ...
    Rep(i,N){
        c = f[N+i-1]/f[i]/f[N-1];
        ans += dc*(N+i)*c*PowMod(da.a,N)*PowMod(db.a,i);
        ans += dc*(N+i)*c*PowMod(db.a,N)*PowMod(da.a,i);
    }
