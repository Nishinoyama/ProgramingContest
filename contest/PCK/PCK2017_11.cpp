...
int main() {
    ...
    vl ew(N);
    Rep(i,N-1){
        printf ("%lld,%d -v>", i+1, hld.vid[hld.pare[hld.inv[i+1]]]);
        printf ("%d,%d\n", hld.inv[i+1], hld.pare[hld.inv[i+1]]);
        ew[i+1] = e[pii(hld.inv[i+1], hld.pare[hld.inv[i+1]])];
    }
    ...
    SegmentTree<ll,ll> ST(f,g,h,0,0,p);
    ST.build( ew );
